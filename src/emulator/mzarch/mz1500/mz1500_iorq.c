...
uint8_t port_read_cb(z80_t *cpu, uint16_t addr, void *user_data)
{
    ...
    /* MZ-1500: porty 0xF0-0xF1 (joystick) nejsou na MZ-1500 dostupne */
...
void port_write_cb(z80_t *cpu, uint16_t addr, uint8_t value, void *user_data)
{
    ...
    /* MZ-1500: Pezik ramdisk (0xE8, 0xEC-0xEF) neni dostupny — port 0xE9 koliduje s PSG */

    case 0xe9:
        /* MZ-1500: port 0xE9 zapisuje do obou PSG (mono fallback) */
        psg_write_byte(PSG_CH_RIGHT | PSG_CH_LEFT, value);
        TRACELOG_IORQ_MARK_HANDLED();
        break;
    case 0xf2:
        /* PSG0 — levy kanal */
    ...
    case 0xf0:
    case 0xf1:
        /* MZ-1500: 0xF0 = DMD registr, 0xF1 = barevna paleta */
 ...
 * kompilované, takže pro MZ-1500 binárku obsahuje jen MZ_AVAIL_1500
 * entries. Mirror callbacky (PPI Port B + CW, CTC counter + CW,
 * WD279x R/W latches, banking 0xE0-E6 s MZ-1500 SPEC pole, Z80 PIO
 * 0xFC/FD) vrací konzistentní hodnoty s Overview I/O Ports panelem.
 * MZ-1500 nemá joystick na 0xF0/F1 - tyto entries v g_io_ports nejsou
 * (MZ_AVAIL_800 only), helper je nenajde a vrátí false → latch fallback.
 *
