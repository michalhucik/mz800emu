...
static inline void gdg_process_events(void)
{
    ...
    case MZEVENT_GDG_STS_VSYNC_START:
         ...
         * Default trigger=RISING zachova legacy fire-on-vsync-start. */
        if ( g_bp_event_active[ BP_EVENT_GDG_VSYNC ] ) {
    ...
    case MZEVENT_GDG_AFTER_LAST_VISIBLE_PIXEL:
        ...
        /* HWE - HW event BP hooks: hsync (signal level 1) + raster:N
         * (param = beam_row). HSYNC end fire dole v SCREEN_ROW_END. */
    ...
    case MZEVENT_GDG_SCREEN_ROW_END:
        ...
        else
        {
             ...
             * Cursor blink state inkrementován v gdg_on_screen_done_event;
             * fire po každém screen_done zaručí zachycení edge nezávisle
             * na CPU PortC pollingu. Detail viz mz800_gdg_event.c. */
